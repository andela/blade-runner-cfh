...
exports.create = function (req, res) {
  ...
  user.save().then(function (newUser) {
    ...
    var token = _jsonwebtoken2.default.sign({ id: newUser._id }, secret, {
      expiresIn: '3h'
    });
...
exports.search = function (req, res) {
  ...
  (0, _isEmail2.default)(user) ? queryObject.email = user : queryObject.name = user;

  User.find(queryObject, function (error, users) {
    if (error) {
    ...
    if (users.length > 0) {
      var userResponse = users.map(function (_ref) {
        var name = _ref.name,
            email = _ref.email,
            avatar = _ref.avatar,
            username = _ref.username;
        return {
          name: name, email: email, avatar: avatar, username: username
        };
      });
