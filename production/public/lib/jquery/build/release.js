...
steps(initialize, checkGitStatus, tagReleaseVersion, gruntBuild, makeReleaseCopies, setNextVersion, uploadToCDN, pushToGithub, exit);
	...
	Object.keys(releaseFiles).forEach(function (key) {
		...
		if (!isBeta || key !== releaseFile) {
			...
			if (/\.map$/.test(releaseFile)) {
				...
				text = fs.readFileSync(builtFile, "utf8").replace(/"file":"([^"]+)","sources":\["([^"]+)"\]/, "\"file\":\"" + releaseFile.replace(/\.min\.map/, ".min.js") + "\",\"sources\":[\"" + releaseFile.replace(/\.min\.map/, ".js") + "\"]");
