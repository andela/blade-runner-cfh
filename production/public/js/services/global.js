...
angular.module('mean.system').factory('Global', [function () {
  ...
  return {
    getAvatars: function getAvatars() {
      return $q.all([$http.get('/avatars')]).then(function (results) {
  ...
  return {
    userDonated: function userDonated(donationObject) {
      return $q.all([$http.post('/donations', donationObject)]).then(function (results) {
