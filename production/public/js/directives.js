...
angular.module('mean.directives', []).directive('player', function () {
  return {
    restrict: 'EA',
    ...
    link: function link(scope, elem, attr) {
      ...
      scope.$watch('game.state', function () {
        if (scope.game.state === 'winner has been chosen') {
          ...
          var removePunctuation = function removePunctuation(cardIndex) {
            ...
            } else if ((cardText.indexOf('!', cardText.length - 2) === cardText.length - 1 || cardText.indexOf('?', cardText.length - 2) === cardText.length - 1) && cardIndex === curQ.numAnswers - 1) {
