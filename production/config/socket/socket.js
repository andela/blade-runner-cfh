...
module.exports = function (io) {
  ...
  var joinGame = function joinGame(socket, data) {
  ...
  var getGame = function getGame(player, socket, requestedGameId, createPrivate) {
    ...
    if (requestedGameId.length && allGames[requestedGameId]) {
      ...
      if (game.state === 'awaiting players' && (!game.players.length || game.players[0].socket.id !== socket.id)) {
  ...
  var fireGame = function fireGame(player, socket) {
    var game;
  ...
  var createGameWithFriends = function createGameWithFriends(player, socket) {
  ...
  var exitGame = function exitGame(socket) {
